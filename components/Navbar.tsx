      ...
      <div className="max-w-[1440px] mx-auto px-6 sm:px-12 lg:px-20 xl:px-24">
        <div className="flex justify-between items-center h-16">
          ...
          <div className="hidden lg:flex gap-8 h-full items-center">
              {navLinks.map((link) => {
                const isActive = (pathname === "/" && activeSection === link.id) || (pathname !== "/" && (pathname === link.href || (link.subLinks && pathname.startsWith(link.href))));
                  ...
                  return (
                    <div key={link.name} className="relative group h-full flex items-center">
                      ...
                      <div className="absolute top-full left-0 pt-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 z-50">
                        <div className="bg-white border border-gray-200 shadow-xl min-w-[280px]">
                          {link.subLinks.map((subLink, index) => (
                            <Link
                              key={subLink.name}
                              href={subLink.href}
                              className={`block px-5 py-3.5 text-[13px] font-medium text-gray-500 hover:text-[#0047b3] hover:bg-gray-50 transition-colors ${
