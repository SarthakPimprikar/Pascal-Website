...
const staticProducts = [
  ...
  // --- ROW 2: Longer Descriptions ---
  {
    ...
    shortDescription: "Oil Pumping Station, Oil Filling, Steering Bearing Race Fitment, Chassis Number Marking, Oil Seal Fitment, Nut Crimping, Nut Tightening, etc.",
    iconName: "Cpu",
  },
  {
    title: "Lifters for Assembly Line",
    slug: "lifters-for-assembly-line",
    shortDescription: "Various types of Lifters are provided as per Customer requirements such as Tank lifter, Bearing Lifter, Trailing Arm Lifter, Pot/ Reactor Lifter, etc.",
    iconName: "ArrowUpSquare",
  },
  {
    title: "VIN Number Marking Machines",
    slug: "vin-number-marking-machines",
    shortDescription: "VIN number marking machines with repeat number check functionality for various auto components such as- Engine crankcase, Chassis, 3w Dash-board, Steering Column, etc.",
    iconName: "ScanBarcode",
  },
  {
    title: "Machining Fixtures",
    slug: "machining-fixtures",
    shortDescription: "Hydraulic Machining fixtures are used to accurately locate and securely clamp & support the job on CNC HMC/ VMC machines for Milling, Drilling and Boring operations.",
    iconName: "Settings2",
  }
];
  ...
  return (
    <section id="products" className="py-28 bg-[#f8fafc] relative overflow-hidden border-b border-gray-100">
      ...
      <div className="max-w-[1440px] mx-auto px-6 sm:px-12 lg:px-20 xl:px-24 relative z-10">
        ...
        {/* Section Header */}
        <ScrollReveal direction="up" delay={50} duration={800} className="max-w-3xl mx-auto text-center mb-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 xl:gap-8">
          {displayProducts.map((product, index) => {
            const IconComponent = iconMap[product.iconName || "Settings"] || Settings;
            const staggerDelay = (index % 4) * 100;
            return (
              <ScrollReveal key={index} direction="up" delay={staggerDelay} duration={850} className="h-full">
                <Link
                  href={`/products/${product.slug}`}
                  className="group relative bg-white p-8 rounded-2xl border border-gray-200/60 shadow-sm hover:shadow-2xl transition-all duration-500 overflow-hidden cursor-pointer hover:-translate-y-1.5 flex flex-col justify-between h-full"
                >
